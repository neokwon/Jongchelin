...
class TradingLogic:
    ...
    def calculate_arbitrage(self):
        ...
        bisellupbuy = ((self.bithumb_bid * 0.9996) - (self.upbit_ask * 1.0005)) / (self.upbit_ask * 1.0005) * 100
        upsellbibuy = ((self.upbit_bid * 0.9995) - (self.bithumb_ask * 1.0004)) / (self.bithumb_ask * 1.0004) * 100
    ...
    async def execute_trading_logic(self):
        try:
            ...
            print(f"업비트 KRW 잔액: {upbit_krw}, 업비트 XRP 잔액: {upbit_xrp}")
            print(f"빗썸 KRW 잔액: {bithumb_krw}, 빗썸 XRP 잔액: {bithumb_xrp}")
    ...
    async def log_result(self, message):
        print(message)
    ...
    async def sell_bithumb_buy_upbit(self):
        try:
            ...
            bithumb_sell_result, upbit_buy_result = await asyncio.gather(bithumb_sell_future, upbit_buy_future)

            await self.log_result(
                f"업비트 매수 결과: 시간 {upbit_buy_result['created_at']}, 가격 {upbit_buy_result['price']}, 수량 {upbit_buy_result['volume']}")
            await self.log_result(
                f"빗썸 매도 결과: 시간 {bithumb_sell_result['created_at']}, 가격 {bithumb_sell_result['price']}, 수량 {bithumb_sell_result['volume']}")
    ...
    async def sell_upbit_buy_bithumb(self):
        try:
            ...
            bithumb_buy_result, upbit_sell_result = await asyncio.gather(bithumb_buy_future, upbit_sell_future)

            await self.log_result(
                f"업비트 매도 결과: 시간 {upbit_sell_result['created_at']}, 가격 {upbit_sell_result['price']}, 수량 {upbit_sell_result['volume']}")
            await self.log_result(
                f"빗썸 매수 결과: 시간 {bithumb_buy_result['created_at']}, 가격 {bithumb_buy_result['price']}, 수량 {bithumb_buy_result['volume']}")
...
async def main():
    ...
    # 임계값 설정
    trading_logic.set_thresholds(0.08, 0.08)
    trading_logic.set_additional_thresholds(200, 700, 400000, 1000)
