    ...
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if btc_target_price < btc_current_price and btc_ma5 < btc_current_price:
                if krw_balance > 5000:
                    buy_result = upbit.buy_market_order("KRW-BTC", krw_balance*0.9995)
                    post_message(myToken,"#coin", "BTC buy ")
            ...
            if eth_target_price < eth_current_price and eth_ma5 < eth_current_price:
                if krw_balance > 5000:
                    buy_result = upbit.buy_market_order("KRW-ETH", krw_balance*0.9995)
                    post_message(myToken,"#coin", "ETH buy" )
            ...
            if btc > 0.00008:
                ...
                post_message(myToken,"#coin", "BTC sell")

            # 이더리움 매도
            eth = get_balance("ETH")
            if eth > 0.00008:
                sell_result = upbit.sell_market_order("KRW-ETH", eth*0.9995)
                post_message(myToken,"#coin", "ETH sell")
        time.sleep(1)
