...
while True:
    try:
        ...
        start_time = get_start_time("KRW-ETH")  # 이더리움으로 변경
        end_time = start_time + datetime.timedelta(days=1)

        if start_time < now < end_time - datetime.timedelta(seconds=10):
            target_price = get_target_price("KRW-ETH", 0.5)
            ma5 = get_ma5("KRW-ETH")  # 5일 이동평균선으로 변경
            current_price = get_current_price("KRW-ETH")
            if target_price < current_price and ma5 < current_price:
                krw = get_balance("KRW")
                if krw > 5000:
                    upbit.buy_market_order("KRW-ETH", krw*0.9995)
                    send_telegram_message("이더리움을 구매하였습니다.")
        else:
            eth = get_balance("ETH")  # 이더리움으로 변경
            if eth > 0.00008:
                upbit.sell_market_order("KRW-ETH", eth*0.9995)
                send_telegram_message("이더리움을 판매하였습니다.")
        time.sleep(1)
